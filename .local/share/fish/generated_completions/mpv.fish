...
complete -c mpv -l slang -d 'Equivalent to --alang, for subtitle tracks (default: auto)'
...
complete -c mpv -l subs-with-matching-audio -d 'When autoselecting a subtitle track, select a full/non-forced one even if the…'
complete -c mpv -l subs-fallback -d 'When autoselecting a subtitle track, if no tracks match your preferred langua…'
complete -c mpv -l subs-fallback-forced -d 'When autoselecting a subtitle track, if no tracks match your preferred langua…'
...
complete -c mpv -l drag-and-drop -d '(X11, Wayland and Windows only) Controls the default behavior of drag and dro…'
...
complete -c mpv -l auto-window-resize -d '(Wayland, Win32, and X11) By default, mpv will automatically resize itself if…'
...
complete -c mpv -l demuxer-hysteresis-secs -d 'Once the --demuxer-max-bytes limit is reached, this value can be used to spec…'
...
complete -c mpv -l directory-mode -d 'When opening a directory, open subdirectories recursively, lazily or not at a…'
...
complete -c mpv -l input-cursor-passthrough -l no-input-cursor-passthrough -d '(X11 and Wayland only) Tell the backend windowing system to allow pointer eve…'
...
complete -c mpv -l screenshot-avif-pixfmt -d 'Specify the pixel format to the libavcodec encoder. sp Default: yuv420p'
...
complete -c mpv -l cache-dir -d 'Directory where to create temporary files'
...
complete -c mpv -l icc-cache -d 'Store and load 3D LUTs created from the ICC profile on disk in the cache dire…'
...
complete -c mpv -l watch-later-options-clr -d 'No option will be saved to watch later files'
